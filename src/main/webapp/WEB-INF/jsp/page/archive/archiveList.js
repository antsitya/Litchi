layui.config({
	...
	$("body").on("click",".archive",function(){
		...
		console.log(id);
		var url="/Litchi/archive/toarchiveinfo.do?id="+id;
		console.log("url:"+url);
		...
		function renderDate(data){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+  '<td>'+currData[i].archiveId+'</td>'
			    	+  '<td>'+currData[i].fromId+'</td>'
			    	+  '<td>'+currData[i].didId+'</td>'
//			    	+  '<td>'+1+'</td>'
			    	+  '<td>'+toDate(currData[i].time)+'</td>'
			    	+  '<td>'+currData[i].times+'</td>'
			    	+  '<td>'
					+  '<a class="layui-btn layui-btn-mini archive" data-id="'+currData[i].archiveId+'"><i class="iconfont icon-edit"></i>查看</a>'
			        +  '</td>'
			    	+'</tr>';
				}
			}else{
...
//			console.log("dataHtml:"+dataHtml);
		...
		laypage({
			...
			jump : function(obj){
				...
				$.ajax({
	                url: "/Litchi/archive/archiveList.do",
	                data: {                          //数据
	                	page:obj.curr
	                },
	                type: "POST",//类型，POST或者GET
	                ...
	                success: function (data) {
	                	usersData=data;
	                	$(".archive_content").html(renderDate(usersData));
